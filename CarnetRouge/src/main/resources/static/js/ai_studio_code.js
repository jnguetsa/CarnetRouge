document.addEventListener('DOMContentLoaded', () => {
    ...
    // ══════════════════════════════════════════════
    // ✅ DONNÉES BACKEND — chargées depuis les APIs
    // ══════════════════════════════════════════════

    let TEACHERS = [];  // { id, name, subjects: [{id, nom, code}], color }
    ...
    // ══════════════════════════════════════════════
    // ✅ CHARGEMENT ENSEIGNANTS ET UE depuis le backend
    // ══════════════════════════════════════════════
    async function loadEnseignants() {
        try {
            const res = await fetch('/admin/enseignants/api');
            if (!res.ok) throw new Error('Erreur chargement enseignants');
            const enseignants = await res.json();
            // Transformer pour correspondre au format TEACHERS
            TEACHERS = enseignants.map(e => ({
                id: e.id,
                name: e.nom + ' ' + e.prenom,
                subjects:[],  // sera chargé après
                color: e.couleur || '#dc2626'
            }));
        } catch (err) {
            console.error('Erreur enseignants:', err);
            toast('Erreur chargement enseignants');
        }
    }

    async function loadUEsParEnseignant(enseignantId) {
        try {
            // 👇 CORRECTION ICI : la route correspond maintenant à ton EnseignantController
            const res = await fetch(`/admin/enseignants/${enseignantId}/ues`);
            if (!res.ok) return[];
            return await res.json();
        } catch { return[]; }
    }
    // ══════════════════════════════════════════════
    // ✅ CHARGEMENT EDT depuis le backend (FullCalendar events)
    // ══════════════════════════════════════════════
    async function loadEdtClasse(classeId, start, end) {
        if (!classeId) return[];
        showLoading();
        try {
            const debut = start.toISOString().split('T')[0];
            const fin   = end.toISOString().split('T')[0];
            const res = await fetch(`/admin/emplois-du-temps/api/classe/${classeId}?debut=${debut}&fin=${fin}`);
            ...
            return seances.map(s => ({
                ...
                start:           s.start,   // "2026-03-16T08:00:00"
                end:             s.end,     // "2026-03-16T10:00:00"
                backgroundColor: s.couleur || '#dc2626',
                borderColor:     s.couleur || '#dc2626',
                extendedProps: {
                    dbId:         s.id,
                    enseignant:   s.enseignantNom + ' ' + s.enseignantPrenom,
                    ueNom:        s.ueNom,
                    ueCode:       s.ueCode,
                    salle:        s.salle,
                    classeNom:    s.classeNom,
                    fromBackend:  true   // ✅ distingue les events backend des events drag&drop locaux
                }
            }));
        } catch (err) {
            console.error('Erreur EDT:', err);
            toast('Erreur chargement emploi du temps');
            return[];
        } finally {
            hideLoading();
        }
    ...
    // ══════════════════════════════════════════════
    // ✅ SAUVEGARDE d'une séance vers le backend
    // ══════════════════════════════════════════════
    async function sauvegarderSeance(fcEvent) {
        if (!currentClasseId) {
            toast('⚠ Sélectionnez une classe avant de placer un cours');
            fcEvent.remove();
            return;
        }
        ...
        // Récupérer les infos depuis les props
        const props = fcEvent.extendedProps;
        if (!props.ueId || !props.enseignantId) {
            // C'est un événement custom (pas un cours)
            await sauvegarderEvenement(fcEvent);
            return;
        }
        ...
        try {
            ...
            // ✅ Mettre à jour l'id de l'event avec l'id backend
    ...
    async function sauvegarderEvenement(fcEvent) {
        const start = fcEvent.start;
        const end   = fcEvent.end || new Date(start.getTime() + 3600000);
        ...
        try {
            const body = {
                nom:            fcEvent.title.split('\n')[0],
                description:    fcEvent.extendedProps?.description || '',
                couleur:        fcEvent.backgroundColor || '#dc2626',
                plageHoraireId: null  // sera créé automatiquement
            };
            // Pour les événements simples on crée d'abord la plage horaire
            const plageBody = {
                jour:         start.toISOString().split('T')[0],
                heureDebut:   start.toTimeString().slice(0,8),
                heureFin:     end.toTimeString().slice(0,8),
                ...
                couleur:      fcEvent.backgroundColor || '#dc2626',
                classeId:     currentClasseId,
                ueId:         1,  // à adapter selon ton contexte
                enseignantId: 1
            };
            // Simplifié — ajuste selon tes besoins
            toast('✅ Événement placé');
        } finally {
            hideLoading();
        }
    }
    ...
    // ══════════════════════════════════════════════
    // ✅ SUPPRESSION d'une séance vers le backend
    // ══════════════════════════════════════════════
    async function supprimerSeance(fcEvent) {
        const dbId = fcEvent.extendedProps?.dbId;
        if (!dbId) {
            // Event local, supprimer juste de FullCalendar
            fcEvent.remove();
            return;
        }
        showLoading();
    ...
    // ══════════════════════════════════════════════
    // ✅ MISE À JOUR après drag&drop ou resize
    // ══════════════════════════════════════════════
    async function mettreAJourSeance(fcEvent) {
        const dbId = fcEvent.extendedProps?.dbId;
        if (!dbId || !currentClasseId) return;
        ...
        try {
            const body = {
                ...
                heureDebut:   start.toTimeString().slice(0,8),
                heureFin:     end.toTimeString().slice(0,8),
                salle:        fcEvent.extendedProps?.salle || '',
                couleur:      fcEvent.backgroundColor || '#dc2626',
                classeId:     currentClasseId,
                ueId:         fcEvent.extendedProps?.ueId || 1,
                enseignantId: fcEvent.extendedProps?.enseignantId || 1
            };
    ...
    // ══════════════════════════════════════════════
    // CHIPS ENSEIGNANTS / UE (chargés depuis backend)
    // ══════════════════════════════════════════════
    function buildTeacherChips() {
        const grid = $('teacher-chips');
        if (!TEACHERS.length) {
            grid.innerHTML = '<span style="font-size:11px;color:var(--muted);font-style:italic">Aucun enseignant.</span>';
            return;
        }
        grid.innerHTML = TEACHERS.map(t =>
            `<div class="chip" data-teacher="${t.name}" data-id="${t.id}">${t.name}</div>`
        ).join('');
        grid.querySelectorAll('.chip').forEach(chip => {
            chip.addEventListener('click', async () => {
                const name = chip.dataset.teacher;
                const id   = parseInt(chip.dataset.id);
                if (selTeachers.has(name)) {
                    selTeachers.delete(name);
                    if (!selTeachers.size) selSubject = null;
                } else {
                    selTeachers.add(name);
                    // Charger les UE de cet enseignant si pas encore chargées
                    const teacher = TEACHERS.find(t => t.id === id);
                    if (teacher && !teacher.subjects.length) {
                        teacher.subjects = await loadUEsParEnseignant(id);
                    }
                }
                updateCours();
            });
    ...
    function buildSubjectChips() {
        ...
        const selectedTeacherObjs = TEACHERS.filter(t => selTeachers.has(t.name));
        const available = selectedTeacherObjs.length > 0
            ?[...new Map(selectedTeacherObjs.flatMap(t => t.subjects).map(s => [s.id, s])).values()]
            : TEACHERS.flatMap(t => t.subjects).filter((s, i, arr) => arr.findIndex(x => x.id === s.id) === i);

        if (!available.length) {
            grid.innerHTML = '<span style="font-size:12px;color:var(--muted);font-style:italic">Aucune matière disponible.</span>';
            return;
        }
        grid.innerHTML = available.map(s =>
            `<div class="chip chip-subj${selSubject?.id === s.id ? ' on' : ''}" data-subj="${s.nom}" data-id="${s.id}">${s.nom}</div>`
        ).join('');
        grid.querySelectorAll('.chip-subj').forEach(chip => {
            chip.addEventListener('click', () => {
                const id = parseInt(chip.dataset.id);
                if (selSubject?.id === id) selSubject = null;
                else selSubject = available.find(s => s.id === id);
                updateCours();
            });
        });
    }
    ...
    function updateCours() {
        document.querySelectorAll('#teacher-chips .chip').forEach(c =>
            c.classList.toggle('on', selTeachers.has(c.dataset.teacher))
        );
        buildSubjectChips();
        updateCoursPreview();
        const selectedTeacherObj = TEACHERS.find(t => selTeachers.has(t.name) && t.subjects.some(s => s.id === selSubject?.id));
        $('btn-create-cours').disabled = !(selSubject && selTeachers.size && selectedTeacherObj);
        const info = $('sel-info');
        if (selTeachers.size && selSubject) {
            info.textContent = [...selTeachers].join(' + ') + ' — ' + selSubject.nom;
        } else if (selTeachers.size) {
            info.textContent = selTeachers.size + ' enseignant(s) sélectionné(s)';
    ...
    function updateCoursPreview() {
        const p = $('prev-cours');
        if (selTeachers.size && selSubject) {
            $('pv-t').textContent = selSubject.nom;
            $('pv-s').textContent = [...selTeachers].join(' / ');
            p.style.background = colorCours; p.classList.add('show');
    ...
    $('btn-create-cours').addEventListener('click', () => {
        ...
        cards.push({
            id:          uid(),
            title:       selSubject.nom,
            sub:         [...selTeachers].join(' / ') + (salle ? ' · ' + salle : ''),
            color:       colorCours,
            isPause:     false,
            ueId:        selSubject.id,
            enseignantId: enseignantObj?.id,
            salle:       salle
        });
    ...
    function updateEvPreview() {
        ...
        const p = $('prev-ev');
        if (titre) {
            $('ev-pv-t').textContent = titre;
            $('ev-pv-s').textContent = $('ev-detail').value.trim();
            p.style.background = colorEv; p.classList.add('show');
        } else p.classList.remove('show');
    }
    ...
    function buildMobTeacherChips() {
        ...
        grid.innerHTML = TEACHERS.map(t => `<div class="chip" data-teacher="${t.name}" data-id="${t.id}">${t.name}</div>`).join('');
        grid.querySelectorAll('.chip').forEach(chip => {
            chip.addEventListener('click', async () => {
                const name = chip.dataset.teacher;
                const id = parseInt(chip.dataset.id);
                if (mobSelTeachers.has(name)) { mobSelTeachers.delete(name); if (!mobSelTeachers.size) mobSelSubject = null; }
                    ...
                    mobSelTeachers.add(name);
                    const teacher = TEACHERS.find(t => t.id === id);
                    if (teacher && !teacher.subjects.length) teacher.subjects = await loadUEsParEnseignant(id);
                }
                updateMobCours();
            });
        });
    ...
    function buildMobSubjectChips() {
        ...
        grid.innerHTML = available.map(s =>
            `<div class="chip chip-subj${mobSelSubject?.id === s.id ? ' on' : ''}" data-subj="${s.nom}" data-id="${s.id}">${s.nom}</div>`
        ).join('');
        grid.querySelectorAll('.chip-subj').forEach(chip => {
            chip.addEventListener('click', () => {
                const id = parseInt(chip.dataset.id);
                mobSelSubject = mobSelSubject?.id === id ? null : available.find(s => s.id === id);
                updateMobCours();
            });
        });
    }
    ...
    function updateMobCours() {
        document.querySelectorAll('#mob-teacher-chips .chip').forEach(c => c.classList.toggle('on', mobSelTeachers.has(c.dataset.teacher)));
        buildMobSubjectChips();
        updateMobCoursPreview();
        ...
        else if (mobSelTeachers.size) info.textContent = mobSelTeachers.size + ' enseignant(s) sélectionné(s)';
        else info.textContent = '';
    }

    function updateMobCoursPreview() {
        const p = $('mob-prev-cours');
        if (mobSelTeachers.size && mobSelSubject) {
            $('mob-pv-t').textContent = mobSelSubject.nom;
            $('mob-pv-s').textContent = [...mobSelTeachers].join(' / ');
            p.style.background = mobColorCours; p.classList.add('show');
        } else p.classList.remove('show');
    }
    ...
    // ══════════════════════════════════════════════
    // ✅ FULLCALENDAR avec connexion backend
    // ══════════════════════════════════════════════
    calendar = new FullCalendar.Calendar($('calendar'), {
        ...
        // ✅ Chargement dynamique selon la vue affichée
        datesSet(info) {
            const s = info.start;
            ...
            // ✅ Recharger l'EDT depuis le backend quand on change de semaine
            if (currentClasseId) {
                loadEdtClasse(currentClasseId, info.start, info.end).then(events => {
                    // Supprimer les anciens events backend
                    calendar.getEvents().filter(e => e.extendedProps?.fromBackend).forEach(e => e.remove());
        ...
        // ✅ Quand un event est reçu (drag depuis sidebar), on le sauvegarde
        eventReceive(info) {
            if (!info.event.extendedProps?.isClone) {
                const start = info.event.start;
                info.event.setEnd(new Date(start.getTime() + 3600000));
            }
            // Sauvegarder vers le backend
            sauvegarderSeance(info.event);
        },

        // ✅ Quand on déplace un event existant
        eventDrop(info) {
            mettreAJourSeance(info.event);
        },
    ...
    // ══════════════════════════════════════════════
    // ✅ SÉLECTEUR DE CLASSE — déclenche le chargement de l'EDT
    // ══════════════════════════════════════════════
    $('classe-select').addEventListener('change', async (e) => {
        currentClasseId = e.target.value ? parseInt(e.target.value) : null;
        // Supprimer les events backend existants
        calendar.getEvents().filter(ev => ev.extendedProps?.fromBackend).forEach(ev => ev.remove());

        if (!currentClasseId) return;
        toast('Chargement de l\'emploi du temps…');
        const view = calendar.view;
        const events = await loadEdtClasse(currentClasseId, view.activeStart, view.activeEnd);
        events.forEach(ev => calendar.addEvent(ev));
        toast(`✅ EDT chargé — ${events.length} séance(s)`);
    });
        ...
        const tryAttach = () => {
            ...
            function startDrag(e, isLeft) {
                ...
                function onMove(ev) {
                    ...
                    if (!tCol) return;
                    const startIdx = colRects.findIndex(c => c.date === origStart.toISOString().slice(0,10));
                ...
                function onUp() {
                    ...
                    if (finalStart.getTime() === origStart.getTime() && finalEnd.getTime() === origStart.getTime()) return;
                    const title = currentEvent.title, props = {...currentEvent.extendedProps};
                    const startDateStr = finalStart.toISOString().slice(0,10), endDateStr = finalEnd.toISOString().slice(0,10);
                    ...
                    cols.forEach(col => {
                        ...
                        calendar.addEvent({ id: uid(), title, start: ds, end: new Date(ds.getTime() + durationMs), backgroundColor: color, borderColor: color, extendedProps: {...props, isClone: true} });
                    });
                }
    ...
    $('winput').value = new Date().toISOString().split('T')[0];
    ...
    $('clear-confirm').addEventListener('click', () => {
        ...
        $('clear-bg').classList.remove('on');
        toast('Emploi du temps vidé');
    });
    ...
    $('m-confirm').addEventListener('click', () => {
        if (pendingDel) supprimerSeance(pendingDel);  // ✅ supprime aussi en backend
        $('mbg').classList.remove('on'); pendingDel = null;
    });
    $('mbg').addEventListener('click', e => { if (e.target === $('mbg')) { $('mbg').classList.remove('on'); pendingDel = null; } });

    // ══════════════════════════════════════════════
    // ✅ INITIALISATION — charger enseignants puis chips
    // ══════════════════════════════════════════════
    async function init() {
